...
- **`Otto/`** — native SwiftUI menu-bar app launched by `./run.sh --app`. It's an `LSUIElement` (no Dock icon); its surfaces are the floating command palette and a menu-bar status item. `OttoApp.swift` (the `AppDelegate`) spawns `src/voice_agent.py --ipc` as a subprocess, reads `IPC_PORT=` from its stdout, and connects via `PythonBridge.swift`. `HotkeyManager.swift` registers ⌥Space (Carbon `RegisterEventHotKey`) to toggle the palette. The view controllers each own an `NSWindow`/`NSPanel` driven by SwiftUI: `CommandPalette.swift` (palette + `WaveformView.swift`), `JournalWindow.swift` (capability journal, ⌥⇧Space), `SettingsWindow.swift` (settings — saving restarts the Python subprocess via `AppDelegate.restartPython()` so env-read-at-launch values apply), `MenuBarController.swift` (status item menu: Open Search / Journal / Settings / Quit), and `OnboardingView.swift` + `SetupEngine.swift` (first-run venv+deps install, shown only from the installed bundle when `SettingsStore` isn't configured). `SettingsStore.swift` persists the API key in the Keychain and other prefs in `UserDefaults`, exposing them as env vars for the subprocess. The app finds the repo via `OTTO_PROJECT_ROOT` (legacy `VOICEOS_PROJECT_ROOT` still honored) or by walking up from the bundle, and merges `.env` so it works when launched from Finder.
