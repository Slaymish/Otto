...
make pkg                        # make app, then wrap as Otto/build/Otto.pkg installer (drops into /Applications)
...
- **`Otto/`** — native SwiftUI menu-bar app launched by `./run.sh --app`. It's an `LSUIElement` (no Dock icon); its surfaces are the floating command palette and a menu-bar status item. `OttoApp.swift` (the `AppDelegate`) spawns `src/voice_agent.py --ipc` as a subprocess, reads `IPC_PORT=` from its stdout, and connects via `PythonBridge.swift`. `HotkeyManager.swift` registers global hotkeys (Carbon `RegisterEventHotKey`); summon (default ⌥Space) and journal (default ⌥⇧Space) shortcuts are user-configurable in Settings and persisted via `SettingsStore`. The view controllers each own an `NSWindow`/`NSPanel` driven by SwiftUI: `CommandPalette.swift` (circular orb palette that morphs into a 640pt bar when text is typed, tap-to-toggle mic, `CapabilityHalo` floating chips, `WaveformView.swift` and `OrbView.swift`), `JournalWindow.swift` (capability journal), `SettingsWindow.swift` (settings — saving calls `AppDelegate.restartHotkeys()` and `restartPython()`; hosts Shortcuts, Updates, mic auto-start sections), `MenuBarController.swift` (status item menu: Open Search / Journal / Settings / Update badge / Quit), and `OnboardingView.swift` + `SetupEngine.swift` (first-run venv+deps install, shown only from the installed bundle when `SettingsStore` isn't configured). `SettingsStore.swift` persists the API key in the Keychain and other prefs (`summonHotkey`, `journalHotkey`, `micAutoStart`) in `UserDefaults`, exposing them as env vars for the subprocess. The app finds the repo via `OTTO_PROJECT_ROOT` (legacy `VOICEOS_PROJECT_ROOT` still honored) or by walking up from the bundle, and merges `.env` so it works when launched from Finder.
  - **New Swift files added in the UI/lifecycle overhaul:**
    - `HotkeyConfig.swift` — `struct HotkeyConfig: Codable, Equatable`; Carbon key-code + modifier mask; `summonDefault`/`journalDefault`; `displayString`; `init?(keyCode:modifierFlags:)` (rejects bare keys).
    - `HotkeyRecorderView.swift` — SwiftUI "click to record" control; local key monitor captures next combo with at least one modifier; Esc cancels.
    - `UpdateChecker.swift` — `enum SemVer { static func isNewer }` + `@MainActor @Observable class UpdateChecker`; polls `api.github.com/repos/Slaymish/Otto/releases/latest`; downloads `.pkg` asset and opens macOS Installer; `currentVersion` read from `CFBundleShortVersionString`; daily timer wired in `AppDelegate`.
    - `OrbView.swift` — 140pt circular orb; `RadialWaveform` (`Canvas`+`TimelineView`@30fps) while listening; mic glyph when idle; breathing pulse animation; tap to toggle listening.
    - `CapabilityHalo.swift` — `HaloItem` + `CapabilityHalo`; distributes up to 5 capability chips radially around the orb at `orbDiameter/2 + 64pt`; simple chips run immediately, parameterized ones pre-fill the input bar.
    - `CapabilityKind.swift` — `enum CapabilityKind { static func isParameterized(template:) -> Bool }`; detects `{token}` placeholders via regex.
  - **Standalone Swift unit tests** live in `tests/swift/` and are compiled directly with `swiftc` (no Xcode required): `HotkeyConfigTests.swift`, `UpdateCheckerTests.swift`, `CapabilityKindTests.swift`.
  - **Adding a Swift file:** it must be registered in *two* hand-maintained places — the `SOURCES` list in `Makefile` (the canonical `make app` build) **and** `Otto/Otto.xcodeproj/project.pbxproj` (no file-system-synchronized groups; add a `PBXBuildFile`, a `PBXFileReference`, a group child, and a `PBXSourcesBuildPhase` entry). `make app` is the primary build path; building from the Xcode project requires full Xcode (CLT-only environments have no `xcodebuild`).
