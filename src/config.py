"""
config.py — single source of truth for all Otto tuneable values.
...
Env vars use the ``OTTO_`` prefix. The pre-rebrand ``VOICEOS_`` prefix is still
honored as a fallback (see ``env`` below) so existing ``.env`` files keep working
— set the ``OTTO_`` name to silence the one-time deprecation notice.
"""
...
# ---------------------------------------------------------------------------
# Env reading with legacy fallback
# ---------------------------------------------------------------------------
_warned_legacy: set[str] = set()


def env(suffix: str, default: str | None = None) -> str | None:
    """Read ``OTTO_<suffix>``, falling back to the legacy ``VOICEOS_<suffix>``.

    Honoring the old ``VOICEOS_`` prefix keeps pre-rebrand ``.env`` files working.
    The first time a legacy var is used, a one-line deprecation notice is printed
    to stderr; thereafter it is silent. Returns ``default`` when neither is set.
    """
    new_key = f"OTTO_{suffix}"
    if new_key in os.environ:
        return os.environ[new_key]
    old_key = f"VOICEOS_{suffix}"
    if old_key in os.environ:
        if old_key not in _warned_legacy:
            _warned_legacy.add(old_key)
            print(
                f"[otto] note: {old_key} is deprecated — rename it to {new_key}",
                file=sys.stderr,
            )
...
# OpenAI / model
# ---------------------------------------------------------------------------
MODEL: str = env("MODEL", "gpt-realtime-2")
VOICE: str = env("VOICE", "ballad")
...
#   OTTO_TRANSCRIBE_MODEL=gpt-4o-transcribe        (best accuracy)
#   OTTO_TRANSCRIBE_MODEL=gpt-4o-mini-transcribe   (cheaper, still good)
#   OTTO_TRANSCRIBE_MODEL=whisper-1                (fastest, least accurate)
TRANSCRIBE_MODEL: str = env("TRANSCRIBE_MODEL", "gpt-4o-transcribe")
TRANSCRIBE_LANGUAGE: str = env("TRANSCRIBE_LANGUAGE", "en")
...
# Wake word
# ---------------------------------------------------------------------------
WAKE_WORD: str = env("WAKE_WORD", "hey chat")
...
OWW_MODEL: str = env("OWW_MODEL", "hey_jarvis")
# Detection threshold: lower = more sensitive (more false triggers); higher = stricter.
OWW_THRESHOLD: float = float(env("OWW_THRESHOLD", "0.5"))
OWW_FRAME_MS: int = 80
...
WHISPER_SIZE: str = env("WHISPER", "small.en")

# ---------------------------------------------------------------------------
# Microphone selection
# ---------------------------------------------------------------------------
# Substring match against sounddevice device names (e.g. "Scarlett", "MacBook").
MIC_NAME: str | None = env("MIC")

# ---------------------------------------------------------------------------
# Logging / IPC
# ---------------------------------------------------------------------------
_tmp = tempfile.gettempdir()
EVENT_LOG: str = os.path.join(_tmp, "otto-events.log")
CLAUDE_LOG: str = os.path.join(_tmp, "otto-claude.log")
HUD_FILE: str = os.path.join(_tmp, "otto-hud.json")
IPC_PORT_FILE: str = os.path.join(_tmp, "otto-ipc-port")

# ---------------------------------------------------------------------------
# Browser / external tools
# ---------------------------------------------------------------------------
# Default browser used by web_search (must be AppleScript-scriptable on macOS).
WEB_BROWSER: str = env("BROWSER", "Safari")
...
CLAUDE_PROJECT_HINT: str = env("CLAUDE_PROJECT_HINT", "")
